...
function drawEditor(){const clips=getClips(active);$('#tp-clips').innerHTML=clips.map((c,i)=>`<div class="tp-clip ${i===selected?'selected':''}"><div class="tp-cliptop"><input type="checkbox" data-include="${i}" aria-label="${escape(c.title)}をハイライトに含める" ${c.included?'checked':''}><button class="tp-clipchoose" data-select="${i}" aria-pressed="${i===selected}">${escape(c.title)}<small>${clock(c.start)} — ${clock(c.end)}</small></button><span class="tp-clipnum">${String(i+1).padStart(2,'0')}</span></div>${i===selected?`<div class="tp-trim"><div class="tp-trimfields"><label>開始<input type="text" data-bound="start" value="${clock(c.start)}" aria-label="切り出し開始時刻"></label><label>終了<input type="text" data-bound="end" value="${clock(c.end)}" aria-label="切り出し終了時刻"></label></div><div class="tp-nudge"><button data-nudge="start">開始を10秒前へ</button><button data-nudge="end">終了を10秒後へ</button></div></div>`:''}</div>`).join('');const c=clips[selected];$('#tp-quote').textContent=c.quote;$('#tp-quote-time').textContent=clock(c.speech)+' · 発話例';$('#tp-preview-time').textContent=clock(c.start)+' — '+clock(c.end);$('#tp-preview-image').className='tp-thumb '+videos[active].thumb;$('#tp-full-duration').textContent=videos[active].duration;$('#tp-endtime').textContent=clock(videos[active].seconds);$('#tp-timeline').innerHTML=clips.map((x,i)=>`<button class="tp-mark ${i===selected?'on':''}" data-select="${i}" style="left:${x.start/videos[active].seconds*100}%;width:${(x.end-x.start)/videos[active].seconds*100}%" aria-label="${escape(x.title)} ${clock(x.start)}"></button>`).join('');totals();}
function showReady(){ $('#tp-progress').hidden=true;$('#tp-ready').hidden=false;drawEditor();}
function open(index,view='highlights'){
  active=index;selected=0;const v=videos[index];
...
$('#tp-save').onclick=()=>{
  ...
  try{link.save(v.recordId,{highlights});v.highlights=highlights;v.state='saved';$('#tp-save-status').textContent='ハイライト設定を保存しました。';}
